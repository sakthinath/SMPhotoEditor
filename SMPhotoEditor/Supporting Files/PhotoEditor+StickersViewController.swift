...
extension SMPhotoEditorViewController: StickersViewControllerDelegate {
    ...
    func addGestures(view: UIView) {
        ...
        let panGesture = UIPanGestureRecognizer(target: self,
                                                action: #selector(SMPhotoEditorViewController.panGesture))
        ...
        let pinchGesture = UIPinchGestureRecognizer(target: self,
                                                    action: #selector(SMPhotoEditorViewController.pinchGesture))
        pinchGesture.delegate = self
        view.addGestureRecognizer(pinchGesture)
        
        let rotationGestureRecognizer = UIRotationGestureRecognizer(target: self,
                                                                    action:#selector(SMPhotoEditorViewController.rotationGesture) )
        rotationGestureRecognizer.delegate = self
        view.addGestureRecognizer(rotationGestureRecognizer)
        
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(SMPhotoEditorViewController.tapGesture))
